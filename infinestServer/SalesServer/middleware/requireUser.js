  ...
  try {
    ...
    try {
      ...
      if (decoded.branch_id) {
        req.user = { branch_id: decoded.branch_id, shop_id: decoded.shop_id, isBranch: true, branchName: decoded.name, isAdmin: !!decoded.isAdmin };
        return next();
      }
      req.user = { userId: decoded.userId, shop_id: decoded.shop_id };
      return next();
    } catch (_) {}
    ...
    req.user = { userId, shop_id: String(shop._id) };
    next();
  } catch (err) {
